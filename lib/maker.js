...
const runMarketMaker = ({ tokenA, tokenB }, tolerance = 0.08, nOffers = 5, interval = utils_1.TIMEOUT_MS, side = 'both', amount, uri = env_1.URI) => __awaiter(void 0, void 0, void 0, function* () {
    while (true) {
        yield (0, exports.clearOrderbookForPair)({ tokenA, tokenB });
        if (side === 'buy' || side === 'both') {
            yield (0, exports.postSpread)({ getsToken: tokenA, givesToken: tokenB }, tolerance, nOffers, env_1.SIGNER, amount, uri);
        }
        if (side === 'sell' || side === 'both') {
            yield (0, exports.postSpread)({ getsToken: tokenB, givesToken: tokenA }, tolerance, nOffers, env_1.SIGNER, amount, uri);
        }
