...
const runMarketMaker = ({ tokenA, tokenB }, tolerance = 0.08, nOffers = 5, signer = env_1.SIGNER, interval = utils_1.TIMEOUT_MS, side = 'both', uri, amount) => __awaiter(void 0, void 0, void 0, function* () {
    while (true) {
        yield (0, exports.clearOrderbookForPair)({ tokenA, tokenB }, uri);
        if (side === 'buy' || side === 'both') {
            yield (0, exports.postSpread)({ getsToken: tokenA, givesToken: tokenB }, tolerance, nOffers, signer, amount, uri);
        }
        if (side === 'sell' || side === 'both') {
            yield (0, exports.postSpread)({ getsToken: tokenB, givesToken: tokenA }, tolerance, nOffers, signer, amount, uri);
        }
        yield publishOnce(uri);
        yield (0, utils_1.timeout)(interval);
    }
});
exports.runMarketMaker = runMarketMaker;
class MarketMaker {
    constructor({ uri = env_1.URI, isStarted = true, side = 'both', tokenA, tokenB, numberOfOffers = 5, tolerance = 0.08, interval = utils_1.TIMEOUT_MS, amount, price, signer = env_1.SIGNER, }) {
        Object.assign(this, {
            uri,
            isStarted,
            side,
